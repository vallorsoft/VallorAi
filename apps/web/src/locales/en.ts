import type { Dictionary } from './types'

export const en = {
  common: {
    appName: 'AI Home Designer',
    loading: 'Loading...',
  },
  nav: {
    projects: 'My projects',
    marketplace: 'Marketplace',
    settings: 'Account settings',
  },
  sidebar: {
    projects: 'Projects',
    marketplace: 'Marketplace',
    settings: 'Settings',
    logout: 'Log out',
    openMenu: 'Open menu',
    closeMenu: 'Close menu',
  },
  landing: {
    headerLogin: 'Sign in',
    headerCta: 'Start for free',
    heroTitle: 'Design your home with',
    heroHighlight: 'Artificial Intelligence',
    heroSubtitle:
      'Describe your dream and the AI generates the house plan, following Romanian building regulations. From concept to permit, all in one place.',
    ctaPrimary: 'Design for free',
    ctaSecondary: 'Already have an account? Sign in',
    features: [
      {
        icon: '🏠',
        title: 'AI plan generation',
        desc: 'Describe what you want in simple words and the AI creates the plan in minutes.',
      },
      {
        icon: '📐',
        title: '2D/3D editor',
        desc: 'Edit the plan interactively with our visual editor, add rooms and walls.',
      },
      {
        icon: '⚖️',
        title: 'Romanian regulations',
        desc: 'Automatic validation against P100, NP 011 standards and local rules.',
      },
    ],
    footerTagline: 'Designed for Romania',
  },
  projectsPage: {
    title: 'My projects',
    subtitle: 'Manage and continue your architecture projects',
  },
  projectsGrid: {
    newProject: 'New project',
    modalTitle: 'New project',
    namePlaceholder: 'Project name',
    cancel: 'Cancel',
    create: 'Create',
    creating: 'Creating...',
    status: {
      DRAFT: 'Draft',
      INTERVIEW: 'Interview',
      DESIGNING: 'Designing',
      REVIEW: 'Review',
      COMPLETE: 'Complete',
    },
  },
  projectDetail: {
    notFound: 'Project not found',
    openEditor: 'Open editor',
    aiAssistantTitle: 'AI architect assistant',
    detailsTitle: 'Project details',
    typeLabel: 'Type',
    statusLabel: 'Status',
    styleLabel: 'Style',
    quickActionsTitle: 'Quick actions',
    actionValidateRules: 'Validate rules',
    actionEstimateCost: 'Estimate cost',
    actionExportDxf: 'Export DXF',
  },
  editor: {
    aiAssistantTitle: 'AI assistant',
    propertiesTitle: 'Properties',
    mobileTabChat: 'AI assistant',
    mobileTabPlan: 'Plan',
    mobileTabProperties: 'Properties',
    floorGround: 'Ground floor',
    floorUpper: 'Floor',
    floorBasement: 'Basement',
    toolSelect: 'Select',
    toolAddRoom: 'Add room',
    toolAddWall: 'Add wall',
    toolCostBoq: 'Cost BOQ',
    toolExportPdf: 'Export PDF',
    exportPdfLoading: 'Generating...',
    toolExportIfc: 'Export IFC',
    exportIfcLoading: 'Generating...',
    toolExportPermitDoc: 'DTAC PDF',
    exportPermitDocLoading: 'Generating...',
    toolView2d: '2D',
    toolView3d: '3D',
    addWallHintFirstClick: 'Click for the wall start point',
    addWallHintSecondClick: 'Click for the wall end point',
    selectRoomHint: 'Select a room on the plan',
    selectedRoomLabel: 'Selected room',
    area: 'Area',
    width: 'Width',
    height: 'Height',
    floor: 'Floor',
    deleteRoom: 'Delete room',
    deletingRoom: 'Deleting...',
    emptyCanvasHint: 'Talk to the AI to generate the house plan',
    layerPanel: {
      title: 'Wall assembly',
      selectWallHint: 'Select a wall on the plan',
      loading: 'Loading layers...',
      materialColumn: 'Material',
      thicknessColumn: 'Thickness',
      standardColumn: 'Standard',
      priceColumn: 'Unit price',
      priceUnverifiedNotice:
        'Estimated, unverified price — confirm with a supplier before using it as a quote.',
      tabLayers: 'Layers',
      tabReinforcement: 'Reinforcement',
      functionLabels: {
        STRUCTURAL: 'Structural',
        INSULATION: 'Insulation',
        RENDER: 'Render',
        FINISH: 'Skim coat',
        PAINT: 'Paint',
      },
    },
    wallReinforcement: {
      title: 'Wall reinforcement',
      empty: 'No reinforcement specified for this wall.',
    },
    costBoqPanel: {
      title: 'Bill of quantities',
      grandTotal: 'Grand total',
      subtotalMaterials: 'Materials subtotal',
      subtotalLabor: 'Labor subtotal',
      laborSection: 'Labor',
      taxSection: 'Taxes',
      vatRate: 'VAT 19%',
      vatNote: 'Reduced 5% rate may apply (Romanian Tax Code art. 291) if price ≤ 600,000 RON and usable area ≤ 120 m².',
      loading: 'Computing estimate...',
      empty: 'No data for the estimate yet — add rooms or walls.',
      unverifiedChip: 'Unverified',
      quantityColumn: 'Quantity',
      unitPriceColumn: 'Unit price',
      lineTotalColumn: 'Total',
      standardColumn: 'Standard',
      categories: {
        wall: 'Walls',
        foundation: 'Foundation',
        tieColumn: 'Tie columns',
        centura: 'Ring beams',
        lintel: 'Lintels',
        roof: 'Roof',
        other: 'Other',
      },
    },
    viewer3d: {
      lodLabel: 'Level of detail',
      masonryComputing: 'Generating masonry…',
      brickCountLabel: 'Bricks',
      rebarCountLabel: 'Rebar bars',
      stirrupCountLabel: 'Stirrups',
      tieColumnCountLabel: 'Tie columns',
      centuraCountLabel: 'Ring beams',
      fpsLabel: 'FPS',
      lowPerfNotice: 'Hardware acceleration unavailable — masonry detail is disabled to keep the view smooth',
    },
    structuralInspector: {
      toolFoundation: 'Foundation',
      toolTieColumns: 'Tie columns',
      toolCenturi: 'Ring beams',
      toolRoof: 'Roof',
      toolLintel: 'Lintel',
      toolStaircase: 'Staircase',
      loading: 'Loading…',
      verified: 'Verified',
      unverified: 'Unverified',
      unverifiedNotice:
        'Estimated, unverified value — confirm with a structural engineer before using it in construction drawings.',
      concreteClass: 'Concrete class',
      cover: 'Concrete cover',
      barDiameter: 'Bar diameter',
      barSpacing: 'Bar spacing',
      barCount: 'Bar count',
      role: {
        LONGITUDINAL: 'Longitudinal',
        STIRRUP: 'Stirrup',
        TRANSVERSE: 'Transverse',
      },
      foundation: {
        title: 'Foundation',
        depth: 'Depth',
        width: 'Width',
        assemblyTitle: 'Layers',
        reinforcementTitle: 'Reinforcement',
      },
      tieColumns: {
        title: 'Tie columns',
        empty: 'No tie columns have been generated for this house.',
        category: 'Category',
        floor: 'Floor',
        crossSection: 'Cross-section',
      },
      centuri: {
        title: 'Ring beams',
        empty: 'No ring beams have been generated for this house.',
        wall: 'Wall',
        level: 'Level',
        height: 'Height',
        width: 'Width',
      },
      lintel: {
        title: 'Lintel',
        selectOpeningHint: 'Select a door or window on the plan',
        material: 'Material',
        length: 'Length',
        width: 'Width',
        bearingLength: 'Bearing (per end)',
        prefabricated: 'Prefabricated',
        yes: 'Yes',
        no: 'No',
      },
      roof: {
        title: 'Roof',
        typeLabel: 'Type',
        pitch: 'Pitch',
        overhang: 'Overhang',
        ridgeHeight: 'Ridge height',
        material: 'Material',
        pitchDefaultHint:
          'The default pitch (35°) sits above the NP 057-2002 snow-retention threshold and every Tondach minimum. Changing it clears the verified badge.',
        overhangDefaultHint:
          'The default overhang (0.7 m) is a widely-cited residential convention, not a normativ figure.',
        saving: 'Saving…',
        types: {
          GABLED: 'Gabled',
          HIPPED: 'Hipped',
          FLAT: 'Flat',
          MONOSLOPE: 'Monoslope',
        },
      },
      staircase: {
        title: 'Staircases',
        empty: 'No staircases added. Add a staircase for multi-storey houses.',
        addButton: 'Add staircase',
        floor: 'Floor',
        width: 'Clear width',
        length: 'Flight length',
        riserCount: 'Riser count',
        riserHeight: 'Riser height',
        treadDepth: 'Tread depth',
        horizontalRun: 'Horizontal run',
        blondelCheck: 'Blondel formula (2R+T)',
        blondelTarget: 'Target: ~630 mm',
        handedness: 'Stair hand',
        handednessRight: 'Right',
        handednessLeft: 'Left',
        codeCompliant: 'Compliant with NP 057-2002',
        codeViolation: 'Code violation',
        loading: 'Loading staircases…',
        deleteButton: 'Delete staircase',
      },
    },
  },
  aiChat: {
    greeting1: "Hi! I'm your AI architect assistant.",
    greeting2: 'Tell me about the house you want to design.',
    placeholder: 'Describe your dream home...',
    error: 'Error communicating with the AI. Please try again.',
    quotaExceeded: 'The AI assistant is temporarily unavailable (free quota reached). Please try again later.',
    roomAdded: 'Room added to the plan',
    roomUpdated: 'Room updated in the plan',
    rebuildButton: 'Build plan from conversation',
    rebuilding: 'Building the plan...',
    rebuildDone: 'Plan updated, rooms added',
    rebuildError: 'Failed to build the plan. Please try again.',
  },
  auth: {
    register: {
      title: 'Create your free account',
      subtitle: 'Design your first home with AI',
      nameLabel: 'Full name',
      namePlaceholder: 'John Doe',
      emailLabel: 'Email',
      emailPlaceholder: 'you@example.com',
      passwordLabel: 'Password',
      passwordPlaceholder: 'At least 8 characters',
      submit: 'Create account',
      submitting: 'Creating account...',
      hasAccount: 'Already have an account?',
      loginLink: 'Sign in',
      checkEmailTitle: 'Check your email',
      checkEmailBody:
        'We sent you a confirmation link. Open it to activate your account and sign in.',
      genericError: 'Registration failed',
    },
    login: {
      title: 'Welcome back',
      subtitle: 'Sign in to continue your projects',
      emailLabel: 'Email',
      emailPlaceholder: 'you@example.com',
      passwordLabel: 'Password',
      passwordPlaceholder: '••••••••',
      submit: 'Sign in',
      submitting: 'Signing in...',
      noAccount: "Don't have an account?",
      registerLink: 'Sign up',
      wrongCredentials: 'Incorrect email or password',
      unverified: 'Email not verified',
      resendVerification: 'Resend confirmation email',
      resent: 'Email resent — check your inbox.',
    },
    verifyEmail: {
      invalidLink: 'Invalid link — confirmation token is missing.',
      verifyingTitle: 'Confirming your email...',
      verifyingBody: 'One moment, please.',
      successTitle: 'Account activated!',
      successBody: 'Redirecting you to your projects...',
      errorTitle: 'Confirmation failed',
      genericError: 'Confirmation failed',
      backToLogin: 'Back to sign in',
    },
  },
  validation: {
    nameMin: 'Name must be at least 2 characters',
    emailInvalid: 'Invalid email',
    passwordMin: 'Password must be at least 8 characters',
    passwordRequired: 'Password is required',
  },
  adminAiSettings: {
    title: 'AI settings',
    description:
      'Controls whether the AI assistant can fall back to a paid provider once the free tier (Gemini) quota is exhausted.',
    toggleLabel: 'Allow paid AI providers as a fallback',
    toggleHintOn:
      'On — if the free quota is exhausted and a paid provider is configured, the assistant will keep responding using it.',
    toggleHintOff:
      'Off — if the free quota is exhausted, the assistant shows a temporary-unavailability message instead of using a paid provider.',
    saving: 'Saving...',
    accessDenied: "You don't have access to this page.",
  },
  collaboration: {
    title: 'Collaboration',
    members: 'Team members',
    inviteEmail: 'Email address',
    inviteRole: 'Role',
    inviteButton: 'Invite',
    roleOwner: 'Owner',
    roleEditor: 'Editor',
    roleViewer: 'Viewer',
    removeButton: 'Remove',
    pendingInvite: 'Pending invitation',
    noMembers: 'No members yet',
    toolCollab: 'Team',
  },
  rules: {
    energySection: 'Energy performance',
    energyWallU: 'Exterior wall thermal transmittance',
    fireSection: 'Fire safety',
    fireCompartment: 'Fire compartment area',
    fireStaircase: 'Staircase required',
    accessibilitySection: 'Accessibility',
    accessibilityEntry: 'Entry door width',
    accessibilityBathroom: 'Bathroom area (wheelchair access)',
  },
  tasks: {
    title: 'Tasks',
    empty: 'No tasks yet',
    addButton: 'New task',
    titleLabel: 'Title',
    descriptionLabel: 'Description',
    priorityLabel: 'Priority',
    assigneeLabel: 'Assignee',
    dueDateLabel: 'Due date',
    statusTodo: 'To do',
    statusInProgress: 'In progress',
    statusDone: 'Done',
    statusCancelled: 'Cancelled',
    priorityLow: 'Low',
    priorityMedium: 'Medium',
    priorityHigh: 'High',
    markDone: 'Mark done',
    markInProgress: 'Mark in progress',
    deleteButton: 'Delete',
    overdue: 'Overdue',
    toolTasks: 'Tasks',
    loading: 'Loading...',
    noAssignee: 'Unassigned',
  },
  mep: {
    toolMep: 'MEP',
    title: 'MEP Installations',
    loading: 'Loading MEP points...',
    empty: 'No rooms defined yet. Add rooms to generate MEP installation points.',
    regenerateButton: 'Regenerate MEP',
    regenerating: 'Regenerating...',
    roomLabel: 'Room',
    unknownRoom: 'Unknown room',
    standardColumn: 'Standard',
    countColumn: 'Qty.',
    typeLabels: {
      WATER_SUPPLY: 'Cold water supply',
      HOT_WATER_SUPPLY: 'Hot water supply',
      DRAIN: 'Drain',
      ELECTRICAL_OUTLET: 'Electrical outlet',
      SWITCH: 'Switch',
      LIGHTING_POINT: 'Lighting point',
    },
  },
  marketplace: {
    title: 'Marketplace',
    subtitle: 'Building materials from verified manufacturers',
    allCategories: 'All categories',
    allSuppliers: 'All suppliers',
    categoryLabel: 'Category',
    supplierLabel: 'Supplier',
    noResults: 'No materials found',
    loading: 'Loading...',
    unitPrice: 'Unit price',
    standardRef: 'Standard',
    priceUnverified: 'Price unverified',
    addToProject: 'Add to project',
    selectProject: 'Select project',
    selectWall: 'Select wall',
    confirmAdd: 'Confirm',
    addSuccess: 'Material added to wall',
    categories: {
      BLOCK: 'Masonry blocks',
      INSULATION: 'Insulation',
      RENDER: 'Render / plaster',
      FINISH: 'Interior finish',
      PAINT: 'Paint',
      CONCRETE: 'Concrete',
      REBAR: 'Rebar steel',
      PRECAST: 'Precast elements',
      ROOFING: 'Roofing',
      OTHER: 'Other',
    },
  },
  admin: {
    title: 'Administration',
    dashboardTitle: 'Dashboard',
    usersTitle: 'Users',
    statsUsers: 'Users',
    statsProjects: 'Projects',
    statsHouses: 'Houses',
    statsMaterials: 'Materials',
    userEmail: 'Email',
    userName: 'Name',
    userRole: 'Role',
    userVerified: 'Verified',
    userCreated: 'Registered',
    changeRole: 'Change role',
    roleSaved: 'Role saved',
    accessDenied: 'Access denied — SUPERADMIN role required',
    loading: 'Loading...',
    navDashboard: 'Dashboard',
    navUsers: 'Users',
    navAiSettings: 'AI Settings',
    roles: {
      GUEST: 'Guest',
      USER: 'User',
      CLIENT: 'Client',
      ARCHITECT: 'Architect',
      STRUCTURAL_ENGINEER: 'Structural Engineer',
      MEP_ENGINEER: 'MEP Engineer',
      ELECTRICAL_ENGINEER: 'Electrical Engineer',
      CONTRACTOR: 'Contractor',
      MANUFACTURER: 'Manufacturer',
      SUPPLIER: 'Supplier',
      ADMIN: 'Admin',
      SUPERADMIN: 'Super Admin',
    },
  },
  floorPlanImport: {
    toolImport: 'Import plan',
    modalTitle: 'Import floor plan from image',
    modalSubtitle: 'Upload a photo or scan of an architectural floor plan. AI will detect rooms automatically.',
    dropzone: 'Drag image here or click to select (JPEG, PNG, WebP)',
    dropzoneActive: 'Drop the image...',
    importing: 'Analysing floor plan...',
    success: 'Floor plan imported successfully',
    error: 'Error importing floor plan',
    close: 'Close',
    importButton: 'Import',
    roomsCreated: 'rooms created',
  },
} satisfies Dictionary

import type { Dictionary } from './types'

export const hu = {
  common: {
    appName: 'AI Home Designer',
    loading: 'Betöltés...',
  },
  nav: {
    projects: 'Projektjeim',
    marketplace: 'Piactér',
    settings: 'Fiók beállításai',
  },
  sidebar: {
    projects: 'Projektek',
    marketplace: 'Piactér',
    settings: 'Beállítások',
    logout: 'Kijelentkezés',
    openMenu: 'Menü megnyitása',
    closeMenu: 'Menü bezárása',
  },
  landing: {
    headerLogin: 'Bejelentkezés',
    headerCta: 'Ingyenes kezdés',
    heroTitle: 'Tervezd meg otthonodat',
    heroHighlight: 'Mesterséges Intelligenciával',
    heroSubtitle:
      'Írd le az álmaidat, és az AI elkészíti a ház tervét, betartva a román építési előírásokat. Az ötlettől az engedélyig, egy helyen.',
    ctaPrimary: 'Tervezz ingyen',
    ctaSecondary: 'Már van fiókod? Lépj be',
    features: [
      {
        icon: '🏠',
        title: 'AI tervgenerálás',
        desc: 'Írd le egyszerű szavakkal, mit szeretnél, és az AI pár perc alatt elkészíti a tervet.',
      },
      {
        icon: '📐',
        title: '2D/3D szerkesztő',
        desc: 'Módosítsd a tervet interaktívan a vizuális szerkesztőnkkel, adj hozzá szobákat és falakat.',
      },
      {
        icon: '⚖️',
        title: 'Román előírások',
        desc: 'Automatikus ellenőrzés a P100, NP 011 szabványok és a helyi előírások szerint.',
      },
    ],
    footerTagline: 'Romániának tervezve',
  },
  projectsPage: {
    title: 'Projektjeim',
    subtitle: 'Kezeld és folytasd az építészeti projektjeidet',
  },
  projectsGrid: {
    newProject: 'Új projekt',
    modalTitle: 'Új projekt',
    namePlaceholder: 'A projekt neve',
    cancel: 'Mégse',
    create: 'Létrehozás',
    creating: 'Létrehozás...',
    status: {
      DRAFT: 'Vázlat',
      INTERVIEW: 'Interjú',
      DESIGNING: 'Tervezés',
      REVIEW: 'Felülvizsgálat',
      COMPLETE: 'Kész',
    },
  },
  projectDetail: {
    notFound: 'A projekt nem található',
    openEditor: 'Szerkesztő megnyitása',
    aiAssistantTitle: 'AI építész asszisztens',
    detailsTitle: 'Projekt részletei',
    typeLabel: 'Típus',
    statusLabel: 'Állapot',
    styleLabel: 'Stílus',
    quickActionsTitle: 'Gyors műveletek',
    actionValidateRules: 'Szabályok ellenőrzése',
    actionEstimateCost: 'Költségbecslés',
    actionExportDxf: 'DXF exportálás',
  },
  editor: {
    aiAssistantTitle: 'AI asszisztens',
    propertiesTitle: 'Tulajdonságok',
    mobileTabChat: 'AI asszisztens',
    mobileTabPlan: 'Terv',
    mobileTabProperties: 'Tulajdonságok',
    floorGround: 'Földszint',
    floorUpper: 'Emelet',
    floorBasement: 'Pince',
    toolSelect: 'Kijelölés',
    toolAddRoom: 'Szoba hozzáadása',
    toolAddWall: 'Fal hozzáadása',
    toolCostBoq: 'Költségvetés',
    toolExportPdf: 'PDF export',
    exportPdfLoading: 'Generálás...',
    toolExportIfc: 'IFC export',
    exportIfcLoading: 'Generálás...',
    toolExportPermitDoc: 'DTAC PDF',
    exportPermitDocLoading: 'Generálás...',
    toolView2d: '2D',
    toolView3d: '3D',
    addWallHintFirstClick: 'Kattints a fal kezdőpontjához',
    addWallHintSecondClick: 'Kattints a fal végpontjához',
    selectRoomHint: 'Válassz ki egy szobát a terven',
    selectedRoomLabel: 'Kiválasztott szoba',
    area: 'Terület',
    width: 'Szélesség',
    height: 'Magasság',
    floor: 'Emelet',
    deleteRoom: 'Szoba törlése',
    deletingRoom: 'Törlés...',
    emptyCanvasHint: 'Beszélj az AI-val a ház tervének elkészítéséhez',
    layerPanel: {
      title: 'Fal-szerkezet',
      selectWallHint: 'Válassz ki egy falat a terven',
      loading: 'Rétegek betöltése...',
      materialColumn: 'Anyag',
      thicknessColumn: 'Vastagság',
      standardColumn: 'Szabvány',
      priceColumn: 'Egységár',
      priceUnverifiedNotice:
        'Becsült, nem ellenőrzött ár — árajánlatként való felhasználás előtt egyeztess egy beszállítóval.',
      tabLayers: 'Rétegek',
      tabReinforcement: 'Vasalás',
      functionLabels: {
        STRUCTURAL: 'Teherhordó',
        INSULATION: 'Hőszigetelés',
        RENDER: 'Vakolat',
        FINISH: 'Glett',
        PAINT: 'Festés',
      },
    },
    wallReinforcement: {
      title: 'Fal vasalása',
      empty: 'Ehhez a falhoz nincs megadva vasalás.',
    },
    costBoqPanel: {
      title: 'Költségvetés',
      grandTotal: 'Végösszeg',
      subtotalMaterials: 'Anyag részösszeg',
      subtotalLabor: 'Munkaerő részösszeg',
      laborSection: 'Munkaerő',
      taxSection: 'Adók',
      vatRate: 'ÁFA 19%',
      vatNote: 'Kedvezményes 5% lehetséges (Codul Fiscal art. 291) ha az ár ≤ 600.000 RON és a hasznos alapterület ≤ 120 m².',
      loading: 'Költségvetés számítása...',
      empty: 'Még nincs adat a költségvetéshez — adj hozzá szobákat vagy falakat.',
      unverifiedChip: 'Nem ellenőrzött',
      quantityColumn: 'Mennyiség',
      unitPriceColumn: 'Egységár',
      lineTotalColumn: 'Összesen',
      standardColumn: 'Szabvány',
      categories: {
        wall: 'Falak',
        foundation: 'Alap',
        tieColumn: 'Merevítő oszlopok',
        centura: 'Koszorúk',
        lintel: 'Áthidalók',
        roof: 'Tető',
        other: 'Egyéb munkák',
      },
    },
    viewer3d: {
      lodLabel: 'Részletességi szint',
      masonryComputing: 'Falazat generálása…',
      brickCountLabel: 'Téglák',
      rebarCountLabel: 'Betonacél rudak',
      stirrupCountLabel: 'Kengyerek',
      tieColumnCountLabel: 'Merevítő oszlopok',
      centuraCountLabel: 'Koszorúk',
      fpsLabel: 'FPS',
      lowPerfNotice: 'Hardveres gyorsítás nem érhető el — a falazat-részletek a gördülékenység érdekében ki vannak kapcsolva',
    },
    structuralInspector: {
      toolFoundation: 'Alap',
      toolTieColumns: 'Merevítő oszlopok',
      toolCenturi: 'Koszorúk',
      toolRoof: 'Tető',
      toolLintel: 'Áthidaló',
      toolStaircase: 'Lépcső',
      loading: 'Betöltés…',
      verified: 'Ellenőrzött',
      unverified: 'Nem ellenőrzött',
      unverifiedNotice:
        'Becsült, nem ellenőrzött érték — kiviteli tervben való felhasználás előtt egyeztess mérnökkel.',
      concreteClass: 'Beton szilárdsági osztály',
      cover: 'Betonfedés',
      barDiameter: 'Rúd átmérő',
      barSpacing: 'Rúd távolság',
      barCount: 'Rudak száma',
      role: {
        LONGITUDINAL: 'Hosszirányú',
        STIRRUP: 'Kengyer',
        TRANSVERSE: 'Keresztirányú',
      },
      foundation: {
        title: 'Alap',
        depth: 'Mélység',
        width: 'Szélesség',
        assemblyTitle: 'Rétegek',
        reinforcementTitle: 'Vasalás',
      },
      tieColumns: {
        title: 'Merevítő oszlopok',
        empty: 'Ehhez a házhoz nincs generált merevítő oszlop.',
        category: 'Kategória',
        floor: 'Szint',
        crossSection: 'Keresztmetszet',
      },
      centuri: {
        title: 'Koszorúk',
        empty: 'Ehhez a házhoz nincs generált koszorú.',
        wall: 'Fal',
        level: 'Szint',
        height: 'Magasság',
        width: 'Szélesség',
      },
      lintel: {
        title: 'Áthidaló',
        selectOpeningHint: 'Válassz ki egy ajtót vagy ablakot a terven',
        material: 'Anyag',
        length: 'Hossz',
        width: 'Szélesség',
        bearingLength: 'Feltámaszkodás (végenként)',
        prefabricated: 'Előregyártott',
        yes: 'Igen',
        no: 'Nem',
      },
      roof: {
        title: 'Tető',
        typeLabel: 'Típus',
        pitch: 'Hajlás',
        overhang: 'Ereszkinyúlás',
        ridgeHeight: 'Gerinc magasság',
        material: 'Anyag',
        pitchDefaultHint:
          'Az alapértelmezett hajlás (35°) az NP 057-2002 hófogó küszöb és a Tondach minimum fölött van. Az érték módosítása kikapcsolja az ellenőrzött jelölést.',
        overhangDefaultHint:
          'Az alapértelmezett ereszkinyúlás (0.7 m) elterjedt lakóházi konvenció, nem szabványban rögzített érték.',
        saving: 'Mentés…',
        types: {
          GABLED: 'Nyeregtető',
          HIPPED: 'Kontyolt tető',
          FLAT: 'Lapos tető',
          MONOSLOPE: 'Fél nyeregtető',
        },
      },
      staircase: {
        title: 'Lépcsők',
        empty: 'Nincs hozzáadott lépcső. Több szintes házhoz adj hozzá lépcsőt.',
        addButton: 'Lépcső hozzáadása',
        floor: 'Szint',
        width: 'Szabad szélesség',
        length: 'Kar hossza',
        riserCount: 'Lépőszám',
        riserHeight: 'Emelkedő magasság',
        treadDepth: 'Lépő mélység',
        horizontalRun: 'Vízszintes futás',
        blondelCheck: 'Blondel-képlet (2E+M)',
        blondelTarget: 'Cél: ~630 mm',
        handedness: 'Felszállási irány',
        handednessRight: 'Jobb',
        handednessLeft: 'Bal',
        codeCompliant: 'NP 057-2002 megfelelő',
        codeViolation: 'Szabálysértés',
        loading: 'Lépcsők betöltése…',
        deleteButton: 'Lépcső törlése',
      },
    },
  },
  aiChat: {
    greeting1: 'Szia! Én vagyok az AI építész asszisztensed.',
    greeting2: 'Mesélj arról a házról, amit szeretnél megtervezni.',
    placeholder: 'Írd le álmaid otthonát...',
    error: 'Hiba történt az AI-jal való kommunikáció során. Próbáld újra.',
    quotaExceeded: 'Az AI asszisztens jelenleg nem elérhető (elérte az ingyenes keretet). Próbáld újra később.',
    roomAdded: 'Szoba hozzáadva a tervhez',
    roomUpdated: 'Szoba frissítve a tervben',
    rebuildButton: 'Terv felépítése a beszélgetésből',
    rebuilding: 'Terv felépítése folyamatban...',
    rebuildDone: 'Terv frissítve, szobák hozzáadva',
    rebuildError: 'A terv felépítése sikertelen. Próbáld újra.',
  },
  auth: {
    register: {
      title: 'Ingyenes fiók létrehozása',
      subtitle: 'Tervezd meg első otthonodat AI segítségével',
      nameLabel: 'Teljes név',
      namePlaceholder: 'Kovács János',
      emailLabel: 'Email',
      emailPlaceholder: 'te@pelda.hu',
      passwordLabel: 'Jelszó',
      passwordPlaceholder: 'Minimum 8 karakter',
      submit: 'Fiók létrehozása',
      submitting: 'Fiók létrehozása...',
      hasAccount: 'Már van fiókod?',
      loginLink: 'Jelentkezz be',
      checkEmailTitle: 'Erősítsd meg az email címed',
      checkEmailBody:
        'Elküldtünk egy megerősítő linket. Nyisd meg a fiókod aktiválásához és a bejelentkezéshez.',
      genericError: 'Sikertelen regisztráció',
    },
    login: {
      title: 'Üdvözlünk újra',
      subtitle: 'Jelentkezz be a projektjeid folytatásához',
      emailLabel: 'Email',
      emailPlaceholder: 'te@pelda.hu',
      passwordLabel: 'Jelszó',
      passwordPlaceholder: '••••••••',
      submit: 'Bejelentkezés',
      submitting: 'Bejelentkezés...',
      noAccount: 'Nincs még fiókod?',
      registerLink: 'Regisztrálj',
      wrongCredentials: 'Hibás email vagy jelszó',
      unverified: 'Email nincs megerősítve',
      resendVerification: 'Megerősítő email újraküldése',
      resent: 'Email újraküldve — nézd meg a postaládád.',
    },
    verifyEmail: {
      invalidLink: 'Érvénytelen link — hiányzik a megerősítő token.',
      verifyingTitle: 'Email megerősítése...',
      verifyingBody: 'Egy pillanat, kérjük várj.',
      successTitle: 'Fiók aktiválva!',
      successBody: 'Átirányítunk a projektjeidhez...',
      errorTitle: 'Sikertelen megerősítés',
      genericError: 'A megerősítés sikertelen',
      backToLogin: 'Vissza a bejelentkezéshez',
    },
  },
  validation: {
    nameMin: 'A névnek legalább 2 karakter hosszúnak kell lennie',
    emailInvalid: 'Érvénytelen email cím',
    passwordMin: 'A jelszónak legalább 8 karakter hosszúnak kell lennie',
    passwordRequired: 'A jelszó megadása kötelező',
  },
  adminAiSettings: {
    title: 'AI beállítások',
    description:
      'Szabályozza, hogy az AI asszisztens átválthat-e egy fizetős szolgáltatóra, ha az ingyenes keret (Gemini) elfogyott.',
    toggleLabel: 'Fizetős AI szolgáltatók engedélyezése tartalékként',
    toggleHintOn:
      'Bekapcsolva — ha az ingyenes keret elfogy és egy fizetős szolgáltató be van állítva, az asszisztens azzal folytatja a válaszadást.',
    toggleHintOff:
      'Kikapcsolva — ha az ingyenes keret elfogy, az asszisztens egy ideiglenes elérhetetlenségi üzenetet mutat, ahelyett hogy fizetős szolgáltatót használna.',
    saving: 'Mentés...',
    accessDenied: 'Nincs jogosultságod ehhez az oldalhoz.',
  },
  collaboration: {
    title: 'Együttműködés',
    members: 'Csapattagok',
    inviteEmail: 'Email cím',
    inviteRole: 'Szerepkör',
    inviteButton: 'Meghívás',
    roleOwner: 'Tulajdonos',
    roleEditor: 'Szerkesztő',
    roleViewer: 'Megtekintő',
    removeButton: 'Eltávolítás',
    pendingInvite: 'Függő meghívó',
    noMembers: 'Nincs tag',
    toolCollab: 'Csapat',
  },
  rules: {
    energySection: 'Energiahatékonyság',
    energyWallU: 'Külső fal hőátbocsátási tényezője',
    fireSection: 'Tűzvédelem',
    fireCompartment: 'Tűzszakasz területe',
    fireStaircase: 'Lépcsőház szükséges',
    accessibilitySection: 'Akadálymentesség',
    accessibilityEntry: 'Bejárati ajtó szélessége',
    accessibilityBathroom: 'Fürdőszoba alapterülete (kerekesszékes)',
  },
  tasks: {
    title: 'Feladatok',
    empty: 'Nincs feladat',
    addButton: 'Új feladat',
    titleLabel: 'Cím',
    descriptionLabel: 'Leírás',
    priorityLabel: 'Prioritás',
    assigneeLabel: 'Felelős',
    dueDateLabel: 'Határidő',
    statusTodo: 'Teendő',
    statusInProgress: 'Folyamatban',
    statusDone: 'Kész',
    statusCancelled: 'Törölve',
    priorityLow: 'Alacsony',
    priorityMedium: 'Közepes',
    priorityHigh: 'Magas',
    markDone: 'Kész',
    markInProgress: 'Elkezdve',
    deleteButton: 'Törlés',
    overdue: 'Lejárt',
    toolTasks: 'Feladatok',
    loading: 'Betöltés...',
    noAssignee: 'Nincs felelős',
  },
  mep: {
    toolMep: 'Gépészet',
    title: 'MEP Installációk',
    loading: 'Installációk betöltése...',
    empty: 'Nincs meghatározott helyiség. Adjon hozzá helyiségeket az installációs pontok generálásához.',
    regenerateButton: 'Installációk újragenerálása',
    regenerating: 'Újragenerálás...',
    roomLabel: 'Helyiség',
    unknownRoom: 'Ismeretlen helyiség',
    standardColumn: 'Szabvány',
    countColumn: 'Db.',
    typeLabels: {
      WATER_SUPPLY: 'Hidegvíz',
      HOT_WATER_SUPPLY: 'Melegvíz',
      DRAIN: 'Lefolyó',
      ELECTRICAL_OUTLET: 'Dugalj',
      SWITCH: 'Kapcsoló',
      LIGHTING_POINT: 'Világítási pont',
    },
  },
  marketplace: {
    title: 'Piactér',
    subtitle: 'Ellenőrzött gyártóktól származó építőanyagok',
    allCategories: 'Minden kategória',
    allSuppliers: 'Minden szállító',
    categoryLabel: 'Kategória',
    supplierLabel: 'Szállító',
    noResults: 'Nem találtunk anyagot',
    loading: 'Betöltés...',
    unitPrice: 'Egységár',
    standardRef: 'Szabvány',
    priceUnverified: 'Ár nem ellenőrzött',
    addToProject: 'Projekthez adás',
    selectProject: 'Projekt kiválasztása',
    selectWall: 'Fal kiválasztása',
    confirmAdd: 'Megerősítés',
    addSuccess: 'Anyag hozzáadva a falhoz',
    categories: {
      BLOCK: 'Falazóblokk',
      INSULATION: 'Hőszigetelés',
      RENDER: 'Vakolat',
      FINISH: 'Belső burkolat',
      PAINT: 'Festék',
      CONCRETE: 'Beton',
      REBAR: 'Betonacél',
      PRECAST: 'Előregyártott elem',
      ROOFING: 'Tetőhéjalás',
      OTHER: 'Egyéb',
    },
  },
  admin: {
    title: 'Adminisztráció',
    dashboardTitle: 'Vezérlőpult',
    usersTitle: 'Felhasználók',
    statsUsers: 'Felhasználók',
    statsProjects: 'Projektek',
    statsHouses: 'Épületek',
    statsMaterials: 'Anyagok',
    userEmail: 'E-mail',
    userName: 'Név',
    userRole: 'Szerepkör',
    userVerified: 'Megerősített',
    userCreated: 'Regisztrált',
    changeRole: 'Szerepkör módosítása',
    roleSaved: 'Szerepkör mentve',
    accessDenied: 'Hozzáférés megtagadva — SUPERADMIN szerepkör szükséges',
    loading: 'Betöltés...',
    navDashboard: 'Vezérlőpult',
    navUsers: 'Felhasználók',
    navAiSettings: 'AI beállítások',
    roles: {
      GUEST: 'Vendég',
      USER: 'Felhasználó',
      CLIENT: 'Ügyfél',
      ARCHITECT: 'Építész',
      STRUCTURAL_ENGINEER: 'Statikus mérnök',
      MEP_ENGINEER: 'Gépészeti mérnök',
      ELECTRICAL_ENGINEER: 'Villamosmérnök',
      CONTRACTOR: 'Kivitelező',
      MANUFACTURER: 'Gyártó',
      SUPPLIER: 'Szállító',
      ADMIN: 'Admin',
      SUPERADMIN: 'Super Admin',
    },
  },
  floorPlanImport: {
    toolImport: 'Alaprajz import',
    modalTitle: 'Alaprajz importálása képből',
    modalSubtitle: 'Tölts fel egy alaprajz fotót vagy szkennelt képet. Az AI automatikusan felismeri a helyiségeket.',
    dropzone: 'Húzd ide a képet, vagy kattints a kiválasztáshoz (JPEG, PNG, WebP)',
    dropzoneActive: 'Engedd el a képet...',
    importing: 'Az alaprajz elemzése...',
    success: 'Alaprajz sikeresen importálva',
    error: 'Hiba az alaprajz importálásakor',
    close: 'Bezárás',
    importButton: 'Importálás',
    roomsCreated: 'helyiség létrehozva',
  },
} satisfies Dictionary
